...
try:
    ...
    SETTINGS_PATH = Path.home() / "Documents/AMPIngest"
...
def the_gui(wd, fields, fa):
    ...
    layout = [
        ...
        [
            sg.Listbox(
                values=tuple(fields["tasks"].value.values()),
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, "Exit"):
            ...
            if errors > 0:
                ...
                tasks_chosen = list(
                    set(tuple(fields["tasks"].value.values())).intersection(
                        set(values[fields["tasks"].key])
                    )
                )

                if fields["tasks"].value["Excel Task"] in tasks_chosen:
                    filetype_to_model = {
                        fields["models"].output[i]: values[fields["models"].key[i]][0]
                        for i in range(fa.models_size)
                    }
                else:
                    filetype_to_model=None
   
                if fields["tasks"].value["Email Task"] in tasks_chosen:
...
def check_errors_gui(window, values, fa, fields):
    ...
    if fields["tasks"].value["Copy Task"] in values[fields["tasks"].key]:
        if not (
    ...
    if fields["tasks"].value["Excel Task"] in values[fields["tasks"].key]:
    ...
    if fields["tasks"].value["Email Task"] in values[fields["tasks"].key]:
